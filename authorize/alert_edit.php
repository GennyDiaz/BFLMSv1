...
<?php
    ...
    if($_POST['id']){
        $sqlnotif =  mysqli_query($conn, "SELECT * FROM notification where notification_id = '$id'");
        while($row = mysqli_fetch_assoc($sqlnotif)) {

            $count = mysqli_num_rows($sqlnotif);
    ?>
                                <form method="POST" action="function_alert.php">
                                    <input type="hidden" name="id" value="<?=$row['notification_id']?>"/> 
                                <div class="mb-3">
                                    <label for="sign" class="col-form-label">Sign Level:</label>
                                    <input type="text" class="form-control" id="sign" name="sign" value="<?=$row['sign_name']?>" required/>
                                </div>
                                <div class="mb-3">
                                    <label for="surge" class="col-form-label">Storm Surge:</label>
                                    <input type="text" class="form-control" id="surge" name="surge"  value="<?=$row['surge']?>" required/>
                                </div>
                                <div class="mb-3">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="color" name="color"  value="<?=$row['color']?>" required/>
                                </div>
