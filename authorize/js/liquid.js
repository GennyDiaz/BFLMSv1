...
function renderChart (wrapper, curData){
    ...
    const textSize = 0.95
    const initialCircleThickness = 0.15
    const initialCircleFillGap = 0
    const waveCount = 1
    const circleColor = '#1E90FF'
    const textColor = '#ffffff'
    const waveColor = '#1E90FF'
    const waveTextColor = '#ffffff'
    const textVertPosition = 0.50
  ...
  function getData(value){
    $.ajax({
      url: "http://localhost/bflmsv1/data/Main_data.php",
      method: "GET",
  ...
  document.addEventListener('DOMContentLoaded', () => {
    setInterval(() => {
      getData(data => {
        renderChart(document.querySelector('#liquid_monitoring'), data)//value here from xampp
      })
    }, 1000)
